    ...
    if (count > 0):
            ...
            def update_fun():
                ...
                try:
                    ...
                    if v != 0:
                        dbid = var.fetchall()
                        if (
                                askval == 'name' or askval == 'NAME' or askval == 'address' or askval == 'ADDRESS' or askval == 'contact' or askval == 'CONTACT' or askval == 'ACCOUNT' or askval == 'account'):
                            ...
                            messagebox.showerror("Information", "Enter a valid detail")
                    else:
                        messagebox.showerror("Information", "account not found")
                except:
                    conn.rollback()
                    messagebox.showerror("Information", "Data Transfer Failed")

            upwin = Tk()
            upwin.title("Update here!")
            upwin.geometry("300x300")
            upwin.resizable(False, False)
            lb1 = Label(upwin, text="Enter account").place(x=60, y=13)

            acc = StringVar()
            tb1 = Entry(upwin, textvariable=acc, width=29)
            tb1.place(x=30, y=30)

            asklb = Label(upwin, text="What you want to update?").place(x=60, y=55)
        ...
        def show():
            if True:
                ...
                show.title("Details")
                vary = 80
                headlb = Label(show, text="customer details Details", font="Times 25 bold").grid(row=0, column=1,
                                                                                                 padx=200)
                # -------------------------------Label of the Table---------------------------------#
        ...
        def submit():
            ...
            try:
                var = conn.cursor()
                var.execute(
                    "insert into pnb(account,name,amount,address,contact,pin) values ('" + acc + "','" + nm + "','" + amnt + "','" + addr + "','" + cont + "','" + pi + "')")
                ...
                conn.commit()
                messagebox.showinfo("Information", "account has created")
            except:

                messagebox.showerror("Information", "fail to create account")
            conn.rollback()
        ...
        submitbtn = Button(winn, text="Submit", command=submit, width=10, height=1, font="Times 10", relief=RAISED,
                           bd=10).place(x=50, y=300)
        deletebtn = Button(winn, text="Delete", width=10, height=1, font="Times 10", relief=RAISED, bd=10,
                           command=delete).place(x=200, y=300)

        updatebtn = Button(winn, text="Update", width=10, height=1, font="Times 10", relief=RAISED, bd=10,
                           command=update).place(x=350, y=300)
        showbtn = Button(winn, text="Show All Data", width=10, height=1, font="Times 10", relief=RAISED, bd=10,
                         command=show).place(x=200, y=350)

        messagebox.showinfo("message", "login successful")

    else:
        messagebox.showerror("message", "please enter correct username or password")
...
pwd = StringVar()
tb2 = Entry(mframe, textvariable=pwd, show="*")
tb2.grid(row=2, column=1)

btn1 = Button(mframe, text="Login", font=('italic', 15, 'bold'), bd=5, width=12, fg='black', relief='raise',
              command=login)
